...
Draft: $$L_{\text{credits}} = \text{Total Gross Hub Transaction Value} \times \text{Transaction Tariff Rate} \times \left( \frac{\text{Local Crime Metric}}{\text{Local Police Efficiency} + \epsilon} \right)$$

- **Tariff evasion:** Smuggling rings use unpoliced flight corridors to move refined elements (like `refined_silicon`, `silver` or `helium_3`) directly through local hubs without logging the entries on the digital state ledgers.
