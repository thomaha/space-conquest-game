...
- **Primary material inputs:** `platinum` (for hyper-precise focusing lenses), `graphene` (for massive thermal dissipation arrays) and `silver` (for high-speed optical mirrors).
...
- **Primary material inputs:** `tungsten` (for high-density armor-piercing kinetic slugs), `superconducting_cuprates` (for electromagnetic rail charging) and heavy `steel` rail beds.
...
- **Primary material inputs:** `silicon_carbide` (outer protective tile array), `carbon_nanotubes` (internal tensile structure) and `lead` (for ambient cosmic radiation blocking).
...
- **Dry mass:** 12,000 kg
- **Primary material inputs:** `bio_polymers`, `lead` (for radiation safety during deep-space transit) and `water_ice`.
...
- **Workforce requirement:** Governed during transit by the `bureaucrat` profession to manage demographic records.
...
- **Dry mass:** 12,000 kg
- **Primary material inputs:** `refined_silicon`, `silver`, `gold` and `titanium_aluminide`.
...
- **Dry mass:** 15,000 kg
- **Primary material inputs:** `steel`, `bio_polymers`, `lead` and `oxygen_gas`.
...
- **Size profile:** 8 slots (Medium Infrastructure)
- **Dry mass:** 14,000 kg
- **Primary material inputs:** `silver`, `refined_silicon` and `titanium_aluminide`.
...
- **Size profile:** 8 slots (Medium Utility)
- **Dry mass:** 18,000 kg
- **Primary material inputs:** Heavy `steel` plating, `lead` and `inconel_alloy`.
...
- **Dry mass:** 35,000 kg
- **Primary material inputs:** `titanium_aluminide`, `refined_aluminum` and `silver`.
...
- **Dry mass:** 24,000 kg
- **Primary material inputs:** `bio_polymers`, `silver` and high-complexity electronic nanofabrication matrices.
...
- **Dry mass:** 48,000 kg
- **Primary material inputs:** `refined_iron`, `refined_nickel`, `refined_chromium` and `tungsten`.
...
- **Size profile:** 12 slots (Heavy Production Matrix)
- **Dry mass:** 32,000 kg
- **Primary material inputs:** `refined_aluminum`, `lead` and `refined_sulfur`.
...
- **Size profile:** 14 slots (Heavy Macro-Industrial)
- **Dry mass:** 46,000 kg
- **Primary material inputs:** `steel`, `lead`, `tungsten` and `refined_sulfur`.
...
- **Dry mass:** 21,000 kg
- **Primary material inputs:** `superconducting_cuprates`, `platinum` and `gold`.
...
- **Dry mass:** 24,500 kg
- **Primary material inputs:** `bio_polymers`, `silver` and advanced environmental simulation capsules.
...
- **Size profile:** 14 slots (Heavy Capital Infrastructure)
- **Dry mass:** 40,000 kg
- **Primary material inputs:** `superconducting_cuprates`, `graphene` and `gold`.
...
- **Dry mass:** 22,000 kg
- **Primary material inputs:** `tungsten`, `silver` and `inconel_alloy`.
...
- **Dry mass:** 95,000 kg
- **Primary material inputs:** `tungsten`, `graphene`, `superconducting_cuprates` and `inconel_alloy`.
...
Global plating overlay: Armor layers scale their material costs directly with the total number of connected module slots. As a station expands by adding production foundries or laboratories, the surface area increases, requiring a proportional expenditure of refined materials to maintain full armor coverage.
Passive damage mitigation: Armor acts as the final passive defense layer before a station suffers permanent structural collapse. 
While shields consume active electrical power from Power Modules to absorb hits, armor requires zero energy grid allocation. It relies purely on material properties to naturally absorb or deflect kinetic energy from mass driver slugs and dissipate thermal energy from continuous beam lasers.
Mass and engineering logistics: Applying a thicker or higher-tier armor plating increases the space station’s total Structural Dry Mass according to the weight characteristics of the researched material. This added weight penalizes the station's sub-light handling properties, heavily inflating the universal credit cost or thruster fuel requirements if a Construction Ship attempts to tow, position or relocate the station structure across coordinates.
...
- **Application method:** Global starframe overlay (scales with the total number of connected module slots).
- **Primary material inputs:** 70% `silicon_carbide` + 30% `lead` (for radiation dampening).
